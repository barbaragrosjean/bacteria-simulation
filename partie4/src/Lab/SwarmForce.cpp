...
Vec2d SwarmForce::f(Vec2d position, Vec2d speed) const
{
    ...
    /* Vec2d force((getAppConfig()["swarms"][groupe->getIdentificateur()]["force factor"].toDouble()*((groupe->getPositionLeader().x)-position.x))
             ,getAppConfig()["swarms"][groupe->getIdentificateur()]["force factor"].toDouble()*(((groupe->getPositionLeader()).y-position.y)));*/
