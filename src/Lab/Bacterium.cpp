...
Bacterium::Bacterium(Quantity energie, Vec2d position, Vec2d direction, double rayon, MutableColor couleur, bool abst)
    ...
    , DirectionDeplacement(direction), energie(energie), lenghtLife(sf::Time::Zero)
    , abstinence(abst), mealClock(sf::Time::Zero), useHelp(sf::Time::Zero)
{
...
void Bacterium::takeHelp()
{
    //si il est en collision avec un help il gagne de l'énergie
    if(getAppEnv().helpCollision(*this) and useHelp.asSeconds() >= getConfig()["use help"].toDouble()) {
        useHelp = sf::Time::Zero;
        addEnergy(getConfig()["energy"]["help"].toDouble());
    }
}

void Bacterium::addEnergy(Quantity qt)
{
    energie += qt;
}
