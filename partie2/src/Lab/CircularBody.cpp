...
void CircularBody::setPosition(const Vec2d& c)
{
    centre = c;
};

void CircularBody::setRadius(double r)
{
    rayon = r;
};
...
{
    //true si la distance entre les centres est <= à la somme des rayons
...
{
    //test de la grandeur des rayons
    if(rayon >= other.rayon) {
        //test de la distance des centres
        if(distance(centre, other.centre) <= (rayon - other.rayon)) {
            return true;
        }
    }
